...
class CowsAndBullsGame:
    ...
    def setup_network_game(self, is_server):
        if is_server:
            ...
            print(f"Zahlenformat: {['Binär', 'Oktal', 'Dezimal', 'Hexadezimal'][self.base == 2 and 0 or self.base == 8 and 1 or self.base == 10 and 2 or 3]}")
    ...
    def menu_selection(self, options, prompt):
        ...
        while True:
            ...
            key = self.get_key()
            if key == '\x1b':  # Pfeiltaste
                key = self.get_key()
                if key == '[':
                    key = self.get_key()
                    if key == 'A':  # Pfeil nach oben
                        selected = (selected - 1) % len(options)
                    elif key == 'B':  # Pfeil nach unten
                        selected = (selected + 1) % len(options)
        ...
        for i, player in enumerate(self.players):
            while True:
                try:
                    ...
                    if not self.validate_input(player_number):
                        continue
                    player_number_int = str(player_number)
                    player["target_number"] = player_number_int
                    break
                except ValueError:
                    print(f"Ungültige Eingabe. Bitte gib eine gültige Zahl im Base {self.base} ein.")
            # Konsolenbildschirm löschen (um die Zahlen zu verstecken)
            print("\n" * 200)
    ...
    def update_timer(self):
        while not self._stop_timer_flag and self.players[self.current_player]["time_left"] > 0:
            ...
            if not self.players[self.current_player]["hidden"]:
                ...
                # Save cursor position, update time, then restore cursor position
                print(f"\033[s\r[{time_display}] {player['name']} ({self.attempts[self.current_player]}) \033[u", end="", flush=True)
